...
def load_data():
    ...
    # Prefer first-preference column when available
    ...
    # MANUAL OVERRIDES – force realistic high demand for these schools
    overrides = {
        "100491": {"Apps Received 2025": 169, "Oversub Ratio": 563},  # Oratory Roman Catholic Primary
    ...
    for urn_str, updates in overrides.items():
        mask = df["URN"].astype(str) == urn_str
        if mask.any():
            for col, value in updates.items():
                df.loc[mask, col] = value
            st.info(f"Applied high-demand override for URN {urn_str}")
        else:
            st.warning(f"Could not apply override for URN {urn_str} – school not found in data")
