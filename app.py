...
# ========================================
#  IMD DATA — local CSV lookup (fastest, most reliable)
#  Falls back to API if CSV not present
# ========================================
IMD_CSV_PATH = "imd2019_postcode.csv"
IMD_CSV_GITHUB = "https://raw.githubusercontent.com/Thierry0303/london-catholic-admissions-calculator/main/imd2019_postcode.csv"

@st.cache_data(show_spinner=False)
def load_imd_lookup():
    """
    Loads postcode → IMD decile lookup table.
    Expected columns: pcds (postcode), imd_decile, imd_score
    Source CSV can be generated by running: python download_imd.py
    """
    for path in [IMD_CSV_PATH, IMD_CSV_GITHUB]:
        try:
            df = pd.read_csv(path, usecols=lambda c: c.lower() in ["pcds", "imd_decile", "imd_score", "postcode", "decile", "score"])
            df.columns = [c.lower() for c in df.columns]
            # Normalise column names
            if "pcds" in df.columns:
                df = df.rename(columns={"pcds": "postcode"})
            if "decile" in df.columns and "imd_decile" not in df.columns:
                df = df.rename(columns={"decile": "imd_decile"})
            if "score" in df.columns and "imd_score" not in df.columns:
                df = df.rename(columns={"score": "imd_score"})
            df["postcode"] = df["postcode"].str.strip().str.upper().str.replace(" ", "", regex=False)
...
def fetch_imd(postcode: str):
    ...
    Uses local CSV lookup first, then falls back to API.
    """
    clean = postcode.strip().upper().replace(" ", "")
    
    # Try local CSV first
    lookup = load_imd_lookup()
    if lookup is not None and clean in lookup.index:
        row = lookup.loc[clean]
        decile = row.get("imd_decile")
    ...
    # Fallback: try postcodes.io → LSOA → deprivation API
    import urllib.request, json
    try:
        ...
        lsoa = d1.get("result", {}).get("lsoa") if d1.get("status") == 200 else None
        if lsoa:
            from urllib.parse import quote as urlquote
            url2 = f"https://imd-by-lsoa.opendatacommunities.org/imd/2019?LSOA={urlquote(lsoa)}"
            req2 = urllib.request.Request(url2, headers={"User-Agent": "Mozilla/5.0", "Accept": "application/json"})
            with urllib.request.urlopen(req2, timeout=8) as r:
                d2 = json.loads(r.read())
            decile = d2.get("IMDDecile") or d2.get("imd_decile") or d2.get("Decile")
            score  = d2.get("IMDScore")  or d2.get("imd_score")  or d2.get("Score")
            if decile:
                return {"decile": int(float(decile)), "score": round(float(score), 1) if score else None}
    except Exception:
        pass
